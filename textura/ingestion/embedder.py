...
try:
    ...
    SentenceTransformer = None # Placeholder class or type for type hinting if needed elsewhere
    SENTENCE_TRANSFORMER_AVAILABLE = False # Corrected typo
    print("Warning: sentence_transformers library not found. Embedder will not function fully.")
...
class Embedder:
    ...
    def __init__(self, model_name: str = 'BAAI/bge-small-en-v1.5'):
        ...
        if SENTENCE_TRANSFORMER_AVAILABLE:
            try:
                ...
                print("Warning: Embedder model could not be loaded despite library being available.")
        else:
            self.model = None
            print("Warning: Embedder initialized without a SentenceTransformer model due to missing library.")
    ...
    def generate_embeddings(self, text_chunks: List[str]) -> List[np.ndarray]:
        ...
        """
        if not self.model:
            print("Error: SentenceTransformer model is not available. Cannot generate embeddings.")
            return []
    ...
    if SENTENCE_TRANSFORMER_AVAILABLE:
        ...
        try:
            ...
            if embedder.model:
                ...
                print("Embedder initialized, but the model could not be loaded (e.g. model name error, network issue).")

        except Exception as e:
            print(f"An error occurred during the Embedder example with library available: {e}")
            print("This might be due to model download issues or other runtime problems.")
    else:
        print("sentence_transformers library IS NOT available. Embedder functionality will be limited.")
        ...
        print(f"Attempting to generate embeddings without library: {embeddings_list} (should be empty list)")
