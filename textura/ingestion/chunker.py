...
try:
    ...
    print(
        "PyMuPDFReader not available. Please install pymupdf and llama-index-readers-file."
    )
    ...
    def chunk_file(self, file_path: Path) -> list[str]:
        ...
        if not reader:
            ...
            try:
                ...
                print(
                    f"Unsupported file type '{file_ext}' and failed to read as plain text: {file_path}. Error: {e}"
                )
                return []
        ...
        try:
            ...
            for doc in llama_documents:
                text_chunks.append(
                    doc.get_content()
                )  # doc.text is an alias for doc.get_content()
            return text_chunks
        except Exception as e:
            print(
                f"Error chunking file {file_path} with {reader.__class__.__name__}: {e}"
            )
            return []
