...
from typing import Any  # Import Tuple if using capitalized version
...
def slugify(text: str) -> str:
    ...
    text = re.sub(
        r"[^\w\s-]", "", text.lower()
    )  # Remove special chars except whitespace and hyphens
...
class VaultWriter:
    ...
    def write_event(
        self, event: EventV1
    ) -> tuple[Path, str]:  # Changed to lowercase 'tuple'
        """
        ...
        """
        event_id = (
            f"event_{slugify(event.description)}_{self._generate_unique_id()[:8]}"
        )
    ...
    def write_mystery(
        ...
        """
        mystery_id = (
            f"mystery_{slugify(mystery.question)}_{self._generate_unique_id()[:8]}"
        )
        ...
        content += f"**Context:**\n{mystery.context}\n\n"
        content += (
            f"**Source:** [[{mystery.source_file}]] (Chunk: {mystery.chunk_id})\n\n"
        )
        content += metadata_comment
...
if __name__ == "__main__":
    ...
    print(
        f"Slugify test: 'long description that exceeds seventy five characters limit to check truncation' -> '{slugify('long description that exceeds seventy five characters limit to check truncation')}'"
    )
