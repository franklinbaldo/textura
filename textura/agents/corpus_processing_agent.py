...
class CorpusProcessingAgent:
    ...
    def __init__(self, chunker: Chunker, embedder: Embedder, max_tokens: int = 100000):
        ...
        self.corpus_documents: List[Path] = [] # Stores paths to documents in the corpus
        self.processed_chunks: List[str] = []
        self.corpus_embeddings = None # Will hold embeddings if generated

    def load_corpus(self, document_paths: List[Union[str, Path]]) -> None:
        """
    ...
    def _cosine_similarity(vec1: np.ndarray, vec2: np.ndarray) -> float:
        ...
        if vec1.shape != vec2.shape:
            ...
            print(f"Warning: Shape mismatch in cosine similarity: {vec1.shape} vs {vec2.shape}")
            return 0.0
    ...
    def process_corpus_documents(self) -> None:
        ...
        for i, doc_path in enumerate(self.corpus_documents):
            print(f"Chunking document {i+1}/{len(self.corpus_documents)}: {doc_path.name}...")
            try:
        ...
        if self.processed_chunks:
            print(f"Generating embeddings for {len(self.processed_chunks)} total chunks...")
            try:
                # Ensure embedder is not None and generate_embeddings exists
                if hasattr(self.embedder, 'generate_embeddings'):
                    self.corpus_embeddings = self.embedder.generate_embeddings(self.processed_chunks)
                    print("Embeddings generated successfully.")
                else:
                    print("Embedder or embedding generation method not available. Skipping embedding generation.")
    ...
    def filter_and_sort_corpus(self, task_description: str, relevance_threshold: float = 0.7) -> List[str]:
        """
        ...
        current_total_tokens = sum(self._estimate_token_count(chunk) for chunk in self.processed_chunks)
        print(f"Total estimated tokens before filtering: {current_total_tokens}")

        if self.corpus_embeddings is None or not hasattr(self.embedder, 'generate_embeddings'):
            print("Corpus embeddings not available or embedder misconfigured. Returning all processed chunks.")
            if current_total_tokens > self.max_tokens:
                 print(f"Warning: Unfiltered chunks ({current_total_tokens} tokens) exceed max_tokens ({self.max_tokens}).")
        ...
        try:
            ...
            if not task_embedding_list or task_embedding_list[0] is None:
                print("Failed to generate task embedding. Returning unprocessed chunks (sorted by original order if applicable).")
            ...
            print(f"Error generating task embedding: {e}. Returning unprocessed chunks (sorted by original order if applicable).")
            return self.processed_chunks
        ...
        for chunk, embedding in zip(self.processed_chunks, corpus_embeddings_np):
            if embedding is None or task_embedding.shape != embedding.shape : # Check shape compatibility
                similarity = 0.0 # or some other default for failed/mismatched embeddings
                print(f"Warning: Skipping similarity calculation for a chunk due to embedding issue (shape: {embedding.shape if embedding is not None else 'None'}).")
            else:
        ...
        if current_total_tokens <= self.max_tokens:
            print(f"Total tokens ({current_total_tokens}) are within the max limit ({self.max_tokens}). Returning chunks sorted by relevance and meeting threshold.")
            return [chunk for chunk, score in similarities if score >= relevance_threshold]
        ...
        for chunk, score in similarities:
            if score >= relevance_threshold:
                ...
                if current_filtered_token_count + chunk_token_count <= self.max_tokens:
                    ...
                    print(f"Reached max token limit ({self.max_tokens}). Stopping filtering.")
                    break
            else:
                # Since chunks are sorted, once we hit a chunk below threshold, the rest will also be.
                print(f"Chunks below relevance threshold ({relevance_threshold}). Stopping filtering.")
                break
...
if __name__ == '__main__':
    ...
    try:
        ...
        agent = CorpusProcessingAgent(chunker=default_chunker, embedder=default_embedder)
        ...
        (dummy_corpus_dir / "doc2.txt").write_text("This is a second document for testing corpus mode.")
        (dummy_corpus_dir / "doc3.pdf").write_text("This is a dummy PDF content (not a real PDF).") # Chunker will treat as .txt

        doc_paths = [
            dummy_corpus_dir / "doc1.txt",
            dummy_corpus_dir / "doc2.txt",
            dummy_corpus_dir / "doc3.pdf", # Chunker will handle this as plain text due to its fallback
        ]
        ...
        try:
            ...
            if default_embedder is not None: # Check if embedder was initialized
                 ...
                 can_run_embedding_dependent_code = agent.corpus_embeddings is not None and len(agent.corpus_embeddings) > 0
        ...
        if can_run_embedding_dependent_code:
            print(f"Agent generated {len(agent.corpus_embeddings)} embeddings for {len(agent.processed_chunks)} chunks.")
            task_desc = "A document about testing software and code."
            print(f"Filtering and sorting based on task: '{task_desc}'")
            filtered = agent.filter_and_sort_corpus(task_description=task_desc, relevance_threshold=0.1) # Low threshold for dummy data
            print(f"\n--- Filtered Chunks ({len(filtered)}) ---")
            for i, chunk_text in enumerate(filtered):
                print(f"Chunk {i+1} (tokens: {CorpusProcessingAgent._estimate_token_count(chunk_text)}):\n{chunk_text}\n")
        else:
        ...
        print("Dummy files created for example. Manual cleanup of '_test_corpus' directory may be needed.")

    except Exception as e:
        print(f"Error during CorpusProcessingAgent example: {e}")
        print("This might be due to issues with initializing Chunker/Embedder, "
