...
from textura.extraction.models import (  # Added EventV1, MysteryV1
    EventV1,
    ExtractionItem,
    MysteryV1,
)
...
def init(workspace: str):
    ...
    try:
        ...
        _ = SourceWatcher(
            source_dir=str(workspace_path), workspace_path=str(workspace_path)
        )

        click.echo(
            f"Workspace structure and manifest created successfully in {workspace_path}"
        )
    except OSError as e:
...
@click.option(
    "--source",
    help="Path to the source data directory or a single file to ingest.",
    required=True,
...
def ingest(workspace: str, source: str):
    ...
    click.echo(
        f"Starting ingestion process for source: {source_path} into workspace: {workspace_path}"
    )

    if not workspace_path.exists() or not (workspace_path / "manifest.json").exists():
        click.echo(
            f"Error: Workspace '{workspace_path}' is not initialized or manifest.json is missing. Run 'textura init' first.",
            err=True,
        )
        return

    try:
        source_dir_for_watcher = (
            source_path.parent if source_path.is_file() else source_path
        )
        watcher = SourceWatcher(
            source_dir=str(source_dir_for_watcher), workspace_path=str(workspace_path)
        )

        if source_path.is_file():
            relative_file_path_str = str(
                source_path.relative_to(source_dir_for_watcher)
            )
            if relative_file_path_str in watcher.ingested_files:
                click.echo(
                    f"File '{source_path.name}' has already been ingested. Skipping."
                )
                files_to_process = []
            else:
        ...
        for file_path in files_to_process:
            ...
            if not embeddings or len(embeddings) != len(text_chunks):
                click.echo(
                    f"Error generating embeddings for {file_path.name}. Skipping."
                )
                continue
            click.echo(f"  - Generated {len(embeddings)} embedding(s).")

            relative_path_for_doc_store = str(
                file_path.relative_to(source_dir_for_watcher)
            )
            if backend == "faiss":
                vector_store.add_embeddings(
                    text_chunks, embeddings, source_file=relative_path_for_doc_store
                )
            else:
                vector_store.add_texts(
                    text_chunks, source_file=relative_path_for_doc_store
                )
            click.echo("  - Added embeddings to vector store.")
        ...
        if processed_files_count > 0:
            ...
            click.echo(
                f"Vector store index and document metadata saved in {vector_store.index_dir}"
            )
        else:
        ...
        click.echo(
            f"An unexpected error occurred during the ingestion process: {e}", err=True
        )
        import traceback
...
def extract(workspace: str):
    ...
    if not docs_jsonl_path.exists():
        click.echo(
            f"Error: Document store 'docs.jsonl' not found in '{docs_jsonl_path.parent}'. "
            "Run 'textura ingest' first.",
            err=True,
        )
        return
    ...
    try:
        with open(docs_jsonl_path) as f:
            for line_num, line in enumerate(f):
                ...
                try:
                    ...
                    click.echo(
                        f"Warning: Skipping malformed line {line_num + 1} in docs.jsonl: {e}",
                        err=True,
                    )
                    continue
                ...
                if not chunk_text:
                    click.echo(
                        f"Warning: Skipping document/chunk with ID '{chunk_id}' (line {line_num + 1}) due to missing 'text' field.",
                        err=True,
                    )
                    continue

                click.echo(
                    f"  Processing chunk ID: {chunk_id} (Source: {source_file})..."
                )
        ...
        click.echo(
            f"Metacognitive logs for extraction saved in: {metacog_logger.log_file_path}"
        )

    except Exception as e:
        click.echo(
            f"An unexpected error occurred during the extraction process: {e}", err=True
        )
        import traceback
...
@click.option(
    "--stats",
    is_flag=True,
    help="Print a run statistics summary based on the metacog log.",
)
...
def weave(workspace: str, stats: bool):  # Renamed pipeline arg
    ...
    if not extractions_jsonl_path.exists():
        click.echo(
            f"Error: Extractions file '{extractions_jsonl_path}' not found. "
            "Run 'textura extract' first.",
            err=True,
        )
        return
    ...
    try:
        with open(extractions_jsonl_path) as f:
            for line_num, line in enumerate(f):
                if not line.strip():
                    continue
                ...
                try:
                    ...
                    click.echo(
                        f"Warning: Skipping malformed line {line_num + 1} in {extractions_jsonl_path}: {e}",
                        err=True,
                    )
                    continue
                ...
                if (
                    "description" in extraction_data and "timestamp" in extraction_data
                ):  # Likely EventV1
                    try:
                        ...
                        click.echo(
                            f"Error processing potential EventV1 object (line {line_num + 1}): {e}\nData: {extraction_data}",
                            err=True,
                        )

                elif (
                    "question" in extraction_data and "context" in extraction_data
                ):  # Likely MysteryV1
                    try:
                        ...
                        click.echo(
                            f"Error processing potential MysteryV1 object (line {line_num + 1}): {e}\nData: {extraction_data}",
                            err=True,
                        )

                if not item_processed:
                    click.echo(
                        f"Warning: Could not determine type for extraction on line {line_num + 1}. Data: {extraction_data}",
                        err=True,
                    )
        ...
        click.echo(
            f"Wrote {written_to_vault_count} items to the vault at '{vault_writer.vault_path}'."
        )
        ...
        if not all_events:
            ...
            timeline_builder = TimelineBuilder(
                workspace_path=str(workspace_path), events=all_events
            )
        ...
        click.echo(
            f"An unexpected error occurred during the weave process: {e}", err=True
        )
        import traceback
