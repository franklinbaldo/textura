...
def mock_embedder(monkeypatch):
    ...
    mock_embeddings_map = {
        ...
        "Doc2 chunk2 tech related.": np.array([0.9, 0.2, 0.1, 0.0, 0.0]), # Most relevant
        "Default chunk for doc3.txt": np.array([0.6, 0.4, 0.4, 0.4, 0.4]), # Adjusted for clearer similarity
...
def corpus_agent(mock_chunker, mock_embedder) -> CorpusProcessingAgent:
    return CorpusProcessingAgent(chunker=mock_chunker, embedder=mock_embedder, max_tokens=30) # Small max_tokens for testing filtering
...
def test_process_corpus_documents(corpus_agent, dummy_doc_paths):
    ...
    expected_chunks = [
        "Doc1 chunk1 tech.", "Doc1 chunk2 general.",
        "Doc2 chunk1 finance.", "Doc2 chunk2 tech related.",
...
def test_estimate_token_count():
    ...
    assert CorpusProcessingAgent._estimate_token_count("   Leading spaces") == 2 # split() behavior
...
def test_cosine_similarity():
    ...
    assert CorpusProcessingAgent._cosine_similarity(vec1, vec_zero) == pytest.approx(0.0)


def test_filter_and_sort_corpus_under_max_tokens(corpus_agent, dummy_doc_paths, monkeypatch):
    # Mock token count to be always small so all chunks pass the token limit
    monkeypatch.setattr(CorpusProcessingAgent, "_estimate_token_count", lambda slf, text: 1) # Added slf
    ...
    filtered_chunks = corpus_agent.filter_and_sort_corpus(DUMMY_TASK_DESCRIPTION, relevance_threshold=0.05)
...
def test_filter_and_sort_corpus_with_filtering_and_token_limit(corpus_agent, dummy_doc_paths, monkeypatch):
    ...
    def mock_token_counts(slf, text: str) -> int: # Restored slf
        if "Doc2 chunk2 tech related." in text: return 15 # Takes up half the budget
        if "Doc1 chunk1 tech." in text: return 10      # Takes most of the rest
        if "Default chunk for doc3.txt" in text: return 10 # Would exceed with the above two
        if "Doc1 chunk2 general." in text: return 4
        if "Doc2 chunk1 finance." in text: return 3
        return 1 # default
    monkeypatch.setattr(CorpusProcessingAgent, "_estimate_token_count", mock_token_counts)
    ...
    filtered_chunks = corpus_agent.filter_and_sort_corpus(DUMMY_TASK_DESCRIPTION, relevance_threshold=0.25)
...
def test_filter_and_sort_corpus_relevance_threshold(corpus_agent, dummy_doc_paths, monkeypatch):
    monkeypatch.setattr(CorpusProcessingAgent, "_estimate_token_count", lambda slf, text: 1) # all chunks are small, added slf
    ...
    filtered_chunks = corpus_agent.filter_and_sort_corpus(DUMMY_TASK_DESCRIPTION, relevance_threshold=0.35)
