...
from tests.mocks.mock_llm_service import (
    MockLLMService,  # For patching ExtractorAgent's LLM
)
from textura.cli import textura_cli
from textura.extraction import (
    extractor_agent,  # To allow patching its DEFAULT mock_llm_client
)
...
def test_cli_init_command(runner: CliRunner, test_workspace: Path):
    ...
    assert (
        f"Initializing Textura workspace at {test_workspace.resolve()}" in result.output
    )
...
def test_cli_e2e_ingest_extract_weave(
    runner: CliRunner, test_workspace: Path, test_source_dir: Path
):
    """
    ...
    result_init = runner.invoke(
        textura_cli, ["init", "--workspace", str(test_workspace)]
    )
    ...
    with open(test_workspace / "index" / "docs.jsonl") as f:
        docs = [json.loads(line) for line in f if line.strip()]
    assert len(docs) == 1
    assert docs[0]["text"] == "This is a simple text for E2E test."

    # 3. Extract
    result_extract = runner.invoke(
        textura_cli, ["extract", "--workspace", str(test_workspace)]
    )
    assert result_extract.exit_code == 0
    assert (
        "Successfully extracted 1 items" in result_extract.output
    )  # Based on mock_llm_for_e2e
    ...
    # 4. Weave
    result_weave = runner.invoke(
        textura_cli, ["weave", "--workspace", str(test_workspace), "--stats"]
    )
    ...
    )  # For visibility in pytest output if needed
