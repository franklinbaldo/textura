...
class MockLLMService:
    ...
    """
    def __init__(self):
        self.responses: Dict[str, Union[str, Dict[str, Any]]] = {}
    ...
    def set_response(self, input_text_key: str, response: Union[str, Dict[str, Any]]):
        """
    ...
    def __call__(self, chunk_text: str) -> str:
        ...
        if chunk_text in self.responses:
            ...
            response_data = self.default_responses[self.call_count % len(self.default_responses)]
        else:
            # Fallback if no specific or default responses are set
            response_data = {"extractions": [{"type": "event", "data": {"description": "Fallback response: No specific mock found."}}]}
...
if __name__ == '__main__':
    ...
    mock_service.set_response(test_input1, {
        "extractions": [
    ...
    print(f"Response for '{test_input5}' (default 1 again): {mock_service(test_input5)}")
